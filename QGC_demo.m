...
load ('QGC.mat');% sample containning Quinine,Geranoil and Camphene and solvent peaks have been removed 
 ...
 % ------------------------------  LRSpILT  ---------------------------------
   ...
   mu = 1;
    for k = 1:size(K,2)
       ...
       [X_SPLRA,Conv,Nu,L1,Fn,out] = ADMMBased_Solver(rdt,K,X_ori,lambdalr,lambdasp,mu);
...
%% ---------------------------------------------------------------------------
      ...
      contour(ax2,cs,difCoef,DOSY_X,'LevelList',[0.04:0.1:1]);xlabel('Chemical Shift/ppm');ylabel('Diffusion Coefficient/10^-^1^0m^2s^-^1');
